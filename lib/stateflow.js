...
function State(config, name, parent) {
     ...
     * @property {string|action|flowDefinition} action - the state action to be executed when the state becomes active
 ...
 * @param name {string} the name to be used in the state action property
 * @param action {function|string|object} called when the state becomes active
 * @param initializer {function} called when the flow starts
 * @param destructor {function} called when the flow exits
 * @memberof StateFlow {
 */
...
/**
 * Create a completion function action parameter callback, the callback moves the flow to the next state when an on event matches an event.
